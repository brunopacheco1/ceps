...
public class CepDAO extends AbstractDAO<Cep> {
	...
	public Boolean existsByCEP(String cep) {
		Long result = manager.createQuery("select count(c) from Cep c where c.cep = :cep", Long.class).setParameter("cep", cep).getSingleResult();
		
		return result > 0;
	}
	
	public Cep buscarByCEP(String cep) {
		return manager.createQuery("select c from Cep c where c.cep = :cep", Cep.class).setParameter("cep", cep).getSingleResult();
	}
}